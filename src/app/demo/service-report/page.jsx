...
// Función para eliminar etiquetas de citación
...
export default function ServiceReportPage() {
  ...
  // 1. Lógica de Folio Secuencial desde Supabase
  useEffect(() => {
  ...
  // 4. Subida a Storage y Guardado en DB
  ...
  return (
    <>
      <div className="no-print min-h-screen p-8" style={{ background: "#2F7A4A" }}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-xl p-6 mb-6 shadow-md">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase">Folio Secuencial</label>
                <input value={form.folio} disabled className="w-full border p-2 rounded bg-gray-50 font-bold text-red-600"/>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase">Fecha Emisión</label>
                <input value={form.fecha} disabled className="w-full border p-2 rounded bg-gray-50"/>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              <div className="bg-white p-6 rounded-xl shadow-md">
                <h3 className="font-bold mb-3 text-green-800">INFORMACIÓN DEL CLIENTE</h3>
                ...
                <input placeholder="Dirección / Unidad" value={form.direccion} onChange={e=>handleInput("direccion", e.target.value)} className="w-full border p-2 rounded mb-3"/>
              ...
              {/* Sección de Equipos */}
              <div className="bg-white p-6 rounded-xl shadow-md">
                <h3 className="font-bold mb-3 text-green-800">EQUIPOS ATENDIDOS</h3>
                {form.equipos.map((eq, i) => (
                  <div key={i} className="border p-4 rounded mb-4 bg-gray-50 relative">
                    <div className="grid grid-cols-2 gap-2">
                      <input placeholder="Equipo" value={eq.equipo} onChange={e=>updateEquipo(i, "equipo", e.target.value)} className="border p-2 rounded"/>
                      <input placeholder="Marca" value={eq.marca} onChange={e=>updateEquipo(i, "marca", e.target.value)} className="border p-2 rounded"/>
                      <input placeholder="Modelo" value={eq.modelo} onChange={e=>updateEquipo(i, "modelo", e.target.value)} className="border p-2 rounded"/>
                      <input placeholder="Serie" value={eq.serie} onChange={e=>updateEquipo(i, "serie", e.target.value)} className="border p-2 rounded"/>
                    </div>
                    {form.equipos.length > 1 && <button onClick={()=>removeEquipo(i)} className="text-red-500 text-xs mt-2 underline">Quitar Equipo</button>}
                  </div>
                ))}
                <button onClick={addEquipo} className="text-blue-700 font-bold text-sm">+ AGREGAR OTRO EQUIPO</button>
              </div>
            </div>

            {/* Checklist Anexo */}
            <div className="bg-white p-6 rounded-xl shadow-md h-fit">
              <h3 className="font-bold mb-3 text-green-800">ANEXO TÉCNICO</h3>
              <div className="max-h-[500px] overflow-y-auto pr-2">
                {checklistAire.map((item, idx)=>(
                  <label key={idx} className="flex justify-between items-center py-2 border-b text-sm cursor-pointer hover:bg-gray-50">
                    <span>{item}</span>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md mt-6">
            <h3 className="font-bold mb-4 text-center border-b pb-2 text-green-800">EVIDENCIA FOTOGRÁFICA</h3>
            <div className="grid grid-cols-4 gap-4">
              {['Antes', 'Durante', 'Despues', 'Etiqueta'].map(tipo => (
                <div key={tipo} className="border p-3 rounded-lg bg-gray-50">
                  <p className="font-bold mb-2 text-[10px] uppercase text-center">{tipo}</p>
                  <input type="file" multiple={tipo !== 'Etiqueta'} onChange={e=>handleFotos(e.target.files, `fotos${tipo}`)} className="text-[9px] mb-2 w-full"/>
                  <div className="space-y-2">
                    {form[`fotos${tipo}`]?.map((f, i) => (
                      <div key={i} className="relative aspect-video border rounded bg-white">
                        ...
                        <button onClick={()=>removeFoto(`fotos${tipo}`, i)} className="absolute -top-1 -right-1 bg-red-600 text-white rounded-full w-5 h-5 text-xs">×</button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {/* Botones Principales (MOVIDOS AL FINAL Y CENTRADOS) */}
          <div className="flex justify-center gap-6 mt-12 pb-10">
            <button 
              onClick={guardarReporte} 
              disabled={loading}
              className="bg-blue-700 text-white font-bold py-4 px-12 rounded-full shadow-xl hover:bg-blue-800 disabled:opacity-50 transition-transform hover:scale-105 flex items-center"
            >
      ...
      {/* --- VISTA DE IMPRESIÓN (PDF OFICIAL) --- */}
      <div className="print-only">
        {/* ... (Se mantiene igual que tu código anterior) ... */}
        <div className="print-page">
          ...
          <div className="content-padding">
            <div className="flex justify-between items-end border-b-2 border-black pb-1 mb-4">
              <div>
                <h1 className="text-xl font-bold uppercase leading-tight">Orden/Reporte de Servicio</h1>
                <p className="text-[8px]">{limpiarCitas("FORMATO: MH-MPM-02-01-Folio-Reporte de Servicio")}</p>
              </div>
              <div className="text-right">
                <p className="text-red-600 font-bold text-lg leading-none">{limpiarCitas(form.folio)}</p>
                <p className="text-[9px] font-bold">Fecha: {limpiarCitas(form.fecha)}</p>
              </div>
            </div>
            ...
            <table className="w-full text-[10px] mb-4">
              <tbody>
                <tr className="h-6">
                  ...
                  <td className="border-b border-black">{limpiarCitas(form.cliente)}</td>
                  <td className="font-bold w-32 pl-4 uppercase">Contrato:</td>
                  <td className="border-b border-black">{limpiarCitas(form.contrato)}</td>
                </tr>
                <tr className="h-6">
                  <td className="font-bold uppercase">Dirección:</td>
                  <td className="border-b border-black">{limpiarCitas(form.direccion)}</td>
                  <td className="font-bold pl-4 uppercase">Partida:</td>
                  <td className="border-b border-black">{limpiarCitas(form.partida)}</td>
                </tr>
              </tbody>
            </table>
            ...
            <table className="w-full border-collapse border border-black text-[8px] mt-8 avoid-break text-center">
              <tbody>
                <tr className="h-20">
                  ...
                  <td className="border border-black p-2 w-1/4 align-top text-gray-300 italic">Sello de la Unidad</td>
                </tr>
              </tbody>
            </table>
          </div>
